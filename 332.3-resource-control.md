...
pam\_tally2 module comes in two parts, one is **pam\_tally2.so** and another is **pam\_tally2**. It is based on **PAM** module and can be used to examine and manipulate the counter file. It can display user login attempts counts, set counts on individual basis, unlock all user counts. By default, pam\_tally2 module is already installed on the most of the Linux distributions and it is controlled by **PAM** package itself.
