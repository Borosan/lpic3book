...
|            |                                                                                                                                                                                                                                            |   |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | - |
| snort      | If you run snort without any parameters you can think of it like tcpdump. It would dump bunch of network traffic for you.                                                                                                                  |   |
| snort -D   | It is also possible to run snort as a daemon and it would actively monitor our  networks and analyze it based on rules we have installed.                                                                                                  |   |
| snort-stat | snort-stat is a small script that **generates a nice statistic on the port scans that were detected by snort and reported via /var/log/snort/alert** . The output of this script is mailed to the user confgured in the /etc/snort/snort.  |   |
...
Here we install OpenVas on Rocky linux using gvm. The GVM (Formerly: Openvas) project is an Atomicorp designed effort to install and configure the Openvas vulnerability scanner Version 21.04 on a Redhat, Rocky, Centos or Fedora Linux platforms.
...
{% hint style="danger" %}
&#x20;Selinux is set to (Enforcing) selinux must be disabled in order to use openvas....
{% endhint %}
...
./
404.inc
          3,644 100%    3.48MB/s    0:00:00 (xfr#1, ir-chk=5472/5474)
AfterLogic_WebMail_Pro_detect.nasl
          4,016 100%  784.38kB/s    0:00:00 (xfr#2, ir-chk=5471/5474)
...
The script greenbone-nvt-sync will fetch all new and updated security checks and install them at the proper location. Once this is done it will send a signal to the OpenVAS Scanner, openvassd(8) so that the new NVTs are loaded and considered for new security scans.
