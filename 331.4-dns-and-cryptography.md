...
{% hint style="success" %}
That was a quick overview , to review visit my LPIC2 book: [https://borosan.gitbook.io/lpic2-exam-guide/2071-basic-dns-server-configuration#whats-is-dns](https://borosan.gitbook.io/lpic2-exam-guide/2071-basic-dns-server-configuration#whats-is-dns)
{% endhint %}
...
visit my book: [https://borosan.gitbook.io/lpic2-exam-guide/2073-securing-a-dns-server#207.3-securing-a-dns-server](https://borosan.gitbook.io/lpic2-exam-guide/2073-securing-a-dns-server#207.3-securing-a-dns-server) for more information.
{% endhint %}
...
**T**ransaction **sig**natures (TSIG) is a mechanism used to secure DNS messages and to provide secure server-to-server communication (usually between master and slave server, but can be extended for dynamic updates as well). TSIG can protect the following type of transactions between two DNS servers:
...
BIND includes a utility called rndc (**R**emote **N**ame **D**aemon **C**ontrol) which allows command line administration of the named daemon from the localhost or a remote host.
...
for generating rndc.conf and requred key use `rndc-confgen -r /dev/urandom>/etc/bind/rndc.conf` command, after creating that we have to include the file , and use it but that is not our topic for now.
...
Dig stands for (**D**omain **I**nformation **G**roper) is a network administration command-line tool for querying DNS. It is useful for verifying and troubleshooting DNS problems and also to perform DNS lookups and displays the answers that are returned from the DNS, with no options dig find "A" record of a domain:
...
BIND 9.10 contains a new debugging tool that is a successor to `dig`. It is called `delv`  (**D**NS **l**ookup and **v**alidation _utility_).It works very much like `dig`, but it understands DNSSEC better.
...
<table><thead><tr><th>Options</th><th>Description</th><th data-hidden></th></tr></thead><tbody><tr><td><h4>+rtrace</h4></td><td>Just to list all queried RRs; no further DNSSEC details</td><td></td></tr><tr><td><h4>+mtrace</h4></td><td>Same as rtrace but with the full content of all RRs</td><td></td></tr><tr><td><h4>+vtrace</h4></td><td>Tracing the validation process with many additional notes</td><td></td></tr></tbody></table>
...
• **DO** stands for ** **_**D**NSSEC **O**K_ and indicates that a client will understand a DNSSEC response.&#x20;

• **AD** stands for _**A**uthenticated **D**ata_ and is set when data returned by a server that is authenticated with DNSSEC.
{% endhint %}
