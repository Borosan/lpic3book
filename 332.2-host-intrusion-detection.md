...
**A**dvanced **I**ntrusion **D**etection **E**nvironment (AIDE) **** is a powerful open source intrusion detection tool that uses predefined rules to check the integrity of files and directories in the Linux operating system. AIDE has its own database to check the integrity of files and directories.
...
SCAP stands for **S**ecurity **C**ontext **A**utomation **P**rotocol. And that is a community project started by _Redhat_.
...
>
> **email\_addr** Email address to send reports to
...
{% hint style="danger" %}
Do not forget, that it requires some firewall rules to update!
{% endhint %}
...
> **scan\_clamscan** If clamav is installed, maldet will use clamav scanning for improved performance
>
...
```
[root@rocky8 ~]# rkhunter --cronjob --report-warnings-only
.
.
```
...
```
[root@rocky8 ~]# head  -15 /var/log/rkhunter/rkhunter.log
...
```
[root@rocky8 ~]# cat  /etc/audit/auditd.conf
#
...
```
[root@rocky8 ~]# auditctl -w /etc/passwd -p wa -k user-modify
...
```
[root@rocky8 ~]# useradd testuser
```

Finally, check to see if `auditd` logged the change.

```
[root@rocky8 ~]# ausearch -k user-modify
----
...
```
[root@rocky8 ~]# grep  user-modify /var/log/audit/audit.log
...
```
[root@rocky8 ~]# cat /etc/audit/rules.d/audit.rules
...
```
[root@rocky8 ~]# auditctl  -l
-w /etc/passwd -p wa -k user-modify
[root@rocky8 ~]# auditctl  -D
No rules
[root@rocky8 ~]# auditctl  -l
No rules
[root@rocky8 ~]# auditctl -w /etc/passwd -p wa -k user-modify >> /etc/audit/rules.d/audit.rules
[root@rocky8 ~]# auditctl  -l
-w /etc/passwd -p wa -k user-modify
```
...
```
[root@rocky8 ~]# aureport -x
```

some other options:

```
[root@rocky8 ~]# aureport --help
...
```
[root@rocky8 ~]# ls /usr/share/audit/sample-rules/
...
Sending auditd messages to remote systems Install audispd-plugins package To send to auditd on another server: Configure client system Edit /etc/audisp/plugins.d/au\_remote.conf Set active=yes Edit /etc/audisp/audisp-remote.conf Set remote\_server = target-server-hostname Configure auditd server Edit /etc/audi/auditd.conf: Set tcp\_listen\_port = \<port\_num> Note there is not a default port Set SELinux port context for chosen port Restart auditd: requires reboot Sending auditd messages to remote syslogd Edit /etc/audisp/plugins.d/syslog.conf Set active=yes
{% endhint %}
...
[https://goteleport.com/blog/linux-audit/](https://goteleport.com/blog/linux-audit/)
