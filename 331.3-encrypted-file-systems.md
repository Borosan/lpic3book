...
(visit my lpic1 book: [https://borosan.gitbook.io/lpic1-exam-guide/1041-create-partitions-and-filesystems#fdisk](https://borosan.gitbook.io/lpic1-exam-guide/1041-create-partitions-and-filesystems#fdisk)). Now we want to setup a LUKS encrypted volume on that partition. For that we use `cryptsetup`tool.
...
You can easily close the volume using `luksClose` sub-command of `cryptsetup` (just like `luksOpen`), but do not forget to unmount the volume first.&#x20;
...
now we that  have add the key to the volume and that can be used to mount the volume on boot.
